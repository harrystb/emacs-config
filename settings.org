...
#+BEGIN_SRC emacs-lisp
  ...
  (define-key hb-buffer-map "b" '("Switch to buffer" . consult-switch-to-buffer))
...
#+BEGIN_SRC emacs-lisp
  ...
  (define-key hb-blink-watch-map "c" '("Watch custom" . (lambda (&optional cmd) (interactive "sBin Name:") (compile (format "cargo watch -x \"run --bin %s\"" cmd)))))
  (define-key hb-blink-watch-map "t" '("Watch tmpltst" . (lambda () (interactive) (compile "cargo watch -x \"run --bin tmpltst\""))))
  (setq hb-blink-map (make-sparse-keymap))
  (define-key hb-blink-map "c" '("Run custom" . (lambda (&optional cmd) (interactive "sBin name:") (compile (format "cargo run --bin %s" cmd)))))
  (define-key hb-blink-map "t" '("Run tmpltst" . (lambda () (interactive) (compile "cargo run --bin tmpltst"))))
