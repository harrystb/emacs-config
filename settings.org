...
** Setup font
#+BEGIN_SRC emacs-lisp
  (defun hb-setup-fonts ()
    (when (x-list-fonts "JetBrainsMono NF")
      (set-frame-font "JetBrainsMono NF")))
  (if (daemonp)
      (add-hook 'server-after-make-frame-hook 'hb-setup-fonts)
    (hb-setup-fonts))
#+END_SRC
** Fix windows based ssh
#+BEGIN_SRC emacs-lisp
...
#+END_SRC
** Adjust behaviour of compile window
#+BEGIN_SRC emacs-lisp
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package org
    ...
    (setq org-agenda-files (list "~/DropBox/org/" "~/org/" user-emacs-directory "~/dev/blink/")))
...
#+BEGIN_SRC emacs-lisp
  ...
  (define-key hb-file-map "o" '("Open personal org file" . (lambda () (interactive) (find-file "~/Dropbox/org/personal.org"))))
#+END_SRC
** Buffer commands
#+BEGIN_SRC emacs-lisp
...
#+END_SRC
** Manual session saving/loading
#+BEGIN_SRC emacs-lisp
  (setq hb-desktop-map (make-sparse-keymap))
  (define-key hb-desktop-map "s" '("Save session to desktop file" . desktop-save))
  (define-key hb-desktop-map "d" '("Load session from desktop file" . desktop-read))
  (use-package desktop
    :config
    (push "." desktop-path)
    )
...
#+BEGIN_SRC emacs-lisp
  ...
  (define-key global-map (kbd "C-S-h") '("Quickswitch to buffer 5" . (lambda () (interactive) (hb-harpoon-switch 4))))
  (define-key global-map (kbd "C-S-j") '("Quickswitch to buffer 6" . (lambda () (interactive) (hb-harpoon-switch 5))))
  (define-key global-map (kbd "C-S-k") '("Quickswitch to buffer 7" . (lambda () (interactive) (hb-harpoon-switch 6))))
  (define-key global-map (kbd "C-S-l") '("Quickswitch to buffer 8" . (lambda () (interactive) (hb-harpoon-switch 7))))
#+END_SRC
...
*** Compile commands
#+BEGIN_SRC emacs-lisp
  ;; Setup compilation buffers to colourise with ANSI colours
  (setq compilation-environment '("TERM=xterm-256color"))
  (define-advice compilation-filter (:around (f proc string) xterm-color)
    (funcall f proc (xterm-color-filter string)))
  (setq hb-blink-db-map (make-sparse-keymap))
  (define-key hb-blink-db-map "r" '("Diesel Migration Redo" . (lambda () (interactive) (compile (format "cd %s/database && diesel migration redo" (projectile-project-root))))))
  (define-key hb-blink-db-map "f" '("Diesel Migration Forward" . (lambda () (interactive) (compile (format "cd %s/database && diesel migration run" (projectile-project-root))))))
  (define-key hb-blink-db-map "b" '("Diesel Migration Backward" . (lambda () (interactive) (compile (format "cd %s/database && diesel migration revert" (projectile-project-root))))))
  (setq hb-blink-flutter-map (make-sparse-keymap))
  (define-key hb-blink-flutter-map "g" '("FRB Codegen" . (lambda () (interactive) (compile (format "cd %s/mobile && flutter_rust_bridge_codegen generate" (projectile-project-root))))))
  (define-key hb-blink-flutter-map "l" '("Compile App Localisations" . (lambda () (interactive) (compile (format "cd %s/mobile && flutter gen-l10n" (projectile-project-root))))))
  (setq hb-blink-run-map (make-sparse-keymap))
  (define-key hb-blink-run-map "c" '("Run custom" . (lambda (&optional cmd) (interactive "sBin Name:") (compile (format "cd %s && cargo run --bin %s" (projectile-project-root) cmd)))))
  (define-key hb-blink-run-map "r" '("Run routing" . (lambda () (interactive) (compile (format "cd %s && cargo run --bin routing" (projectile-project-root))))))
  (define-key hb-blink-run-map "t" '("Run tmpltst" . (lambda () (interactive) (compile (format "cd %s && cargo run --bin tmpltst" (projectile-project-root))))))
  ...
  (define-key hb-blink-watch-map "r" '("Watch routing" . (lambda () (interactive) (compile (format "cd %s && cargo watch -x \"run --bin routing\"" (projectile-project-root))))))
  ...
  (define-key hb-blink-map "c" '("Build all binaries" . (lambda () (interactive) (compile (format "cd %s && cargo build" (projectile-project-root))))))
  (define-key hb-blink-map "r" (cons "Run commands" hb-blink-run-map))
  (define-key hb-blink-map "w" (cons "Watch commands" hb-blink-watch-map))
  (define-key hb-blink-map "f" (cons "Flutter commands" hb-blink-flutter-map))
  (define-key hb-blink-map "d" (cons "Database commands" hb-blink-db-map))
#+END_SRC
...
#+BEGIN_SRC emacs-lisp
  (use-package evil
    ...
    (evil-define-key '(normal visual motion) 'global (kbd "<leader>d") (cons "Session Management" hb-desktop-map))
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package flutter
    ...
    :hook
    (flutter-test-mode . company-mode)
    :bind (:map hb-blink-flutter-map
...
#+END_SRC
*** GraphViz Documentation extras (org mode)
#+BEGIN_SRC emacs-lisp
...
(global-set-key (kbd "M-k") 'my-increment-number-decimal)
(global-set-key (kbd "M-j") 'my-decrement-number-decimal)
#+END_SRC
