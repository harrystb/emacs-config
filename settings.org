...
#+BEGIN_SRC emacs-lisp
  ...
  (define-key hb-blink-map "r" '("Run routing" . (lambda () (interactive) (compile (format "cd %s && cargo run --bin routing" (projectile-project-root))))))
...
#+BEGIN_SRC emacs-lisp :tangle no
  (use-package treemacs
    ...
    (use-package treemacs-projectile
      :ensure t)
    ; Move bottom-up searching to after top down so that it searches for git first rather than grabbing closests Cargo.toml
